...
function applyUpdate(ast, uint8Buffer, _ref) {
  ...
  uint8Buffer = (0, _helperBuffer.overrideBytesInBuffer)(
    uint8Buffer, // $FlowIgnore: assertHasLoc ensures that
  ...
  var deltaBytes =
    replacementByteArray.length - // $FlowIgnore: assertHasLoc ensures that
...
function applyDelete(ast, uint8Buffer, node) {
  ...
  if (sectionName === "start") {
    ...
    uint8Buffer = (0, _helperWasmSection.removeSections)(
      ast,
      uint8Buffer,
      "start"
    );
...
function applyAdd(ast, uint8Buffer, node) {
  ...
  if (typeof sectionMetadata === "undefined") {
    var res = (0, _helperWasmSection.createEmptySection)(
      ast,
      uint8Buffer,
      sectionName
    );
...
function applyOperations(ast, uint8Buffer, ops) {
  ops.forEach(function(op) {
    ...
    if (state.deltaElements !== 0 && sectionName !== "start") {
      ...
      state.uint8Buffer = (0, _helperWasmSection.resizeSectionVecSize)(
        ast,
        state.uint8Buffer,
        sectionName,
        state.deltaElements
    ...
    if (state.deltaBytes !== 0 && sectionName !== "start") {
      ...
      state.uint8Buffer = (0, _helperWasmSection.resizeSectionByteSize)(
        ast,
        state.uint8Buffer,
        sectionName,
        state.deltaBytes
